...
`useSyncExternalStore`는 React에서 제공하는 훅으로, 주로 외부 저장소를 구독하기 위한 API로 소개된다.
이 때문에 처음에는 이 훅이 하이드레이션 오류를 방지하는 방법으로 언급되는 점이 다소 의아하게 느껴질 수 있다.

그러나 useSyncExternalStore는 초기 설계 단계부터 [서버 렌더링 환경을 고려](https://ko.react.dev/reference/react/useSyncExternalStore#adding-support-for-server-rendering)해 만들어진 훅이며,
서버와 클라이언트에서 각각 다른 스냅샷을 명시적으로 구분할 수 있도록 설계되어 있다.
이러한 특성을 이해하고 적절히 활용하면, 서버–클라이언트 간 상태 불일치로 인한 하이드레이션 오류를 효과적으로 방지하는 데에도 활용할 수 있다.

```tsx
const snapshot = useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot?)
```

먼저 `getServerSnapshot` 함수가 어떤 시점에 실행되는지를 [문서](https://react.dev/reference/react/useSyncExternalStore)를 통해 살펴보자.

> [!NOTE]
>
> `getServerSnapshot` 함수는 다음과 같이 두 가지 상황에서 실행된다.
>
> - HTML을 생성하는 과정에서 서버에서 실행된다.
> - React가 서버에서 생성된 HTML을 가져와 상호작용 가능하게 만드는 과정, 즉 하이드레이션 중에는 클라이언트에서도 실행된다.

이것이 바로 하이드레이션 오류를 피하기 위해 필요한 핵심이다. 클라이언트에서 `useSyncExternalStore`를 사용하는 페이지로 전환하더라도 `getSnapshot`이 즉시 호출되어 클라이언트의 스냅샷을 가져오게 된다.

그렇다면 어떤 ‘스토어’를 구독해야 할까?

다소 이상하게 들릴 수 있지만, 정답은 업데이트가 전혀 발생하지 않는 빈 스토어를 구독하는 것이다.\
`getSnapshot`은 어차피 모든 렌더링 과정에서 매번 평가되기 때문에, React 외부에서 이 컴포넌트로 상태 업데이트를 푸시할 필요가 없다.
즉, 이 경우 `useSyncExternalStore`는 외부 상태를 구독하기 위한 수단이라기보다, 렌더링 시점에 스냅샷을 안전하게 계산하기 위한 메커니즘으로 활용된다.

이 패턴을 사용하면 하이드레이션 과정에서는 서버와 동일한 렌더링 결과를 유지하면서, 클라이언트 전환 이후에만 특정 UI를 렌더링하도록 제어해 브라우저 전용 API에 안전하게 접근할 수 있다.
