...
위 로직은 내부적으로 `B.prototype`이 `a`의 _프로토타입 체인([[Prototype]])_ 에 포함되어 있는지를 검사한다.
...
> el이 속한 document의 window (defaultView)에서 HTMLElement를 가져오고 그 전역 객체의 HTMLElement.prototype과 비교하기 때문에
