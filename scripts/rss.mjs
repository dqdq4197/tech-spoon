...
async function generateRSS(config, articles, page = 'feed.xml') {
  const publishPosts = articles.filter((post) => post.draft !== true)
  // RSS for blog post
  ...
  if (publishPosts.length > 0) {
    for (const tag of Object.keys(tagData)) {
      const filteredPosts = articles.filter((post) => post.tags.map((t) => slug(t)).includes(tag))
